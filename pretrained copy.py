...
def getGenre(filename):
    ...
    embedding_model = TensorflowPredictEffnetDiscogs(graphFilename="essentia_graphfiles/Discogs Effnet BS64 Model.pb",
                                                     output="PartitionedCall:1")
    embeddings = embedding_model(audio)

    model = TensorflowPredict2D(graphFilename="essentia_graphfiles/MTG Genre Classifier.pb")
...
def getGenreMillion(filename):
    audio = MonoLoader(filename=filename, sampleRate=16000, resampleQuality=4)()
    embedding_model = TensorflowPredictMusiCNN(graphFilename="essentia_graphfiles/MusicNN embed.pb", output="model/dense/BiasAdd")
    embeddings = embedding_model(audio)

    model = TensorflowPredict2D(graphFilename="essentia_graphfiles/MNN Genre.pb", input="serving_default_model_Placeholder", output="PartitionedCall")
    predictions = model(embeddings)
...
def getEngagement(filename):
    ...
    model = TensorflowPredict2D(graphFilename="essentia_graphfiles/Effnet Engagement.pb", output="model/Identity")
    predictions = model(embeddings)
...
def getPopularity(filename):
    ...
    model = TensorflowPredict2D(graphFilename="essentia_graphfiles/Effnet Approachability.pb", output="model/Identity")
    predictions = model(embeddings)
    # return the mean of each frame's popularity and penalize a large STD
    return np.mean(predictions) - np.std(predictions)
...
def getVA(filename):
    ...
    model = TensorflowPredict2D(graphFilename="essentia_graphfiles/MNN Valence Arousal.pb", output="model/Identity")
    predictions = model(embeddings)
...
def getDanceability(filename):
    ...
    model = TensorflowPredict2D(graphFilename="essentia_graphfiles/MNN Danceability.pb", output="model/Softmax")
    predictions = model(embeddings)
...
def getMoodGroup(filename):
    ...
    model = TensorflowPredict2D(graphFilename="essentia_graphfiles/MNN Mood Group.pb", input="serving_default_model_Placeholder",
...
def getAggressive(filename):
    ...
    embedding_model = TensorflowPredictEffnetDiscogs(graphFilename="essentia_graphfiles/Discogs Effnet BS64 Model.pb",
                                                     output="PartitionedCall:1")
    embeddings = embedding_model(audio)
    model = TensorflowPredict2D(graphFilename="essentia_graphfiles/Effnet Aggressive.pb", output="model/Softmax")
    predictions = model(embeddings)
...
def getHappy(filename):
    ...
    embedding_model = TensorflowPredictEffnetDiscogs(graphFilename="essentia_graphfiles/Discogs Effnet BS64 Model.pb",
                                                     output="PartitionedCall:1")
    embeddings = embedding_model(audio)
    model = TensorflowPredict2D(graphFilename="essentia_graphfiles/Effnet Happy.pb", output="model/Softmax")
    predictions = model(embeddings)
...
def getRelaxed(filename):
    ...
    embedding_model = TensorflowPredictEffnetDiscogs(graphFilename="essentia_graphfiles/Discogs Effnet BS64 Model.pb",
                                                     output="PartitionedCall:1")
    embeddings = embedding_model(audio)
    model = TensorflowPredict2D(graphFilename="essentia_graphfiles/Effnet Relax.pb", output="model/Softmax")
    predictions = model(embeddings)
    # return the average confidence that it is aggressive or non
...
def getSad(filename):
    ...
    embedding_model = TensorflowPredictEffnetDiscogs(graphFilename="essentia_graphfiles/Discogs Effnet BS64 Model.pb",
                                                     output="PartitionedCall:1")
    embeddings = embedding_model(audio)
    model = TensorflowPredict2D(graphFilename="essentia_graphfiles/Effnet Sad.pb", output="model/Softmax")
    predictions = model(embeddings)
...
def getTimbre(filename):
    ...
    embedding_model = TensorflowPredictEffnetDiscogs(graphFilename="essentia_graphfiles/Discogs Effnet BS64 Model.pb",
                                                     output="PartitionedCall:1")
    embeddings = embedding_model(audio)
    model = TensorflowPredict2D(graphFilename="essentia_graphfiles/Effnet Timbre.pb", output="model/Softmax")
    predictions = model(embeddings)
...
def getAcoustic(filename):
    ...
    model = TensorflowPredict2D(graphFilename="essentia_graphfiles/MNN Acoustic.pb", output="model/Softmax")
    predictions = model(embeddings)
...
def getElectronic(filename):
    ...
    model = TensorflowPredict2D(graphFilename="essentia_graphfiles/MNN Electronic.pb", output="model/Softmax")
    predictions = model(embeddings)
...
def getInstrumental(filename):
    ...
    model = TensorflowPredict2D(graphFilename="essentia_graphfiles/MNN Instrumental.pb", output="model/Softmax")
    predictions = model(embeddings)
...
def getGender(filename):
    ...
    model = TensorflowPredict2D(graphFilename="essentia_graphfiles/MNN Gender.pb", output="model/Softmax")
    predictions = model(embeddings)
